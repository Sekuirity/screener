...
# Unicode symbols for arrows
#UP_ARROW = strYellow(strBold("↓"))
#DOWN_ARROW = strYellow(strBold("↑"))

# Unicode symbols for arrows
#UP_ARROW = strYellow("^")
#DOWN_ARROW = strYellow("v")
#NO_ARROW = strYellow(" ")
...
sound = r"C:\Windows\Media\Alarm05.wav"

# ASCII symbols for arrows in case Unicode isn't supported
#UP_ARROW = "^"
#DOWN_ARROW = "v"


# Function to compare current and previous values and add arrow symbols
def format_value(current, previous):
    global flag  # Explicitly refer to the global flag
    if current > previous:
        if flag == "False":  # Check the flag's state
            flag = "True"
        #winsound.PlaySound(sound, winsound.SND_FILENAME)  # Play the alarm sound
        return f"{current} {UP_ARROW}"
    elif current < previous:
        if flag == "False":
            flag = "True"
        #winsound.PlaySound(sound, winsound.SND_FILENAME)  # Play the alarm sound
        return f"{current} {DOWN_ARROW}"
    else:
        return f"{current} {NO_ARROW}"
...
def print_headers():
    ...
    print(
        strCyan(f"{'N OI CE':<15}") +  # Increase column width
        strCyan(f"{'N OI PE':<15}") +
        strCyan(f"{'N OI Diff':<15}") +
        strCyan(f"{'N CE Tot Vol':<18}") +
        strCyan(f"{'N PE Tot Vol':<18}") +
        strCyan(f"{'N Tot Vol Diff':<18}") +

        strPurple(f"{'BN OI CE':<15}") +
        strPurple(f"{'BN OI PE':<15}") +
        strPurple(f"{'BN OI Diff':<15}") +
        strPurple(f"{'BN CE Tot Vol':<18}") +
        strPurple(f"{'BN PE Tot Vol':<18}") +
        strPurple(f"{'BN Tot Vol Diff':<18}") + 
        strBold(f"{'Current Time':<18}")
    , flush=True)

# Main function to run the script and print OI and volume differences
def run_script():
    print("", flush=True)

    record_count = 0  # To track when to repeat the column headers
    previous_nifty_data = None
    previous_banknifty_data = None
    
    while True:
        clear_console()
        
        try:
            # Fetch current data for Nifty and Bank Nifty
            ...
            # Format the data with comparison to previous values and adding arrows
            if previous_nifty_data and previous_banknifty_data:
                formatted_nifty_total_oi_CE = format_value(nifty_total_oi_CE, previous_nifty_data[0])
                formatted_nifty_total_oi_PE = format_value(nifty_total_oi_PE, previous_nifty_data[1])
                formatted_nifty_oi_diff = format_value(nifty_oi_difference, previous_nifty_data[2])
                formatted_nifty_ce_vol = format_value(nifty_total_vol_CE, previous_nifty_data[3])
                formatted_nifty_pe_vol = format_value(nifty_total_vol_PE, previous_nifty_data[4])
                formatted_nifty_vol_diff = format_value(nifty_vol_difference, previous_nifty_data[5])

                formatted_banknifty_total_oi_CE = format_value(banknifty_total_oi_CE, previous_banknifty_data[0])
                formatted_banknifty_total_oi_PE = format_value(banknifty_total_oi_PE, previous_banknifty_data[1])
                formatted_bn_oi_diff = format_value(banknifty_oi_difference, previous_banknifty_data[2])
                formatted_bn_ce_vol = format_value(banknifty_total_vol_CE, previous_banknifty_data[3])
                formatted_bn_pe_vol = format_value(banknifty_total_vol_PE, previous_banknifty_data[4])
                formatted_bn_vol_diff = format_value(banknifty_vol_difference, previous_banknifty_data[5])
            else:
                # First iteration doesn't have previous data to compare
                formatted_nifty_total_oi_CE = f"{nifty_total_oi_CE}"
                formatted_nifty_total_oi_PE = f"{nifty_total_oi_PE}"
                formatted_nifty_oi_diff = f"{nifty_oi_difference}"
                formatted_nifty_ce_vol = f"{nifty_total_vol_CE}"
                formatted_nifty_pe_vol = f"{nifty_total_vol_PE}"
                formatted_nifty_vol_diff = f"{nifty_vol_difference}"

                formatted_banknifty_total_oi_CE = f"{banknifty_total_oi_CE}"
                formatted_banknifty_total_oi_PE = f"{banknifty_total_oi_PE}"
                formatted_bn_oi_diff = f"{banknifty_oi_difference}"
                formatted_bn_ce_vol = f"{banknifty_total_vol_CE}"
                formatted_bn_pe_vol = f"{banknifty_total_vol_PE}"
                formatted_bn_vol_diff = f"{banknifty_vol_difference}"

            # Print the formatted data with colors
            print(
                strCyan(f"{formatted_nifty_total_oi_CE:<15}") +
                strCyan(f"{formatted_nifty_total_oi_PE:<15}") +
                strCyan(f"{formatted_nifty_oi_diff:<15}") +
                strCyan(f"{formatted_nifty_ce_vol:<18}") +
                strCyan(f"{formatted_nifty_pe_vol:<18}") +
                strCyan(f"{formatted_nifty_vol_diff:<18}") +

                strPurple(f"{formatted_banknifty_total_oi_CE:<15}") +
                strPurple(f"{formatted_banknifty_total_oi_PE:<15}") +
                strPurple(f"{formatted_bn_oi_diff:<15}") +
                strPurple(f"{formatted_bn_ce_vol:<18}") +
                strPurple(f"{formatted_bn_pe_vol:<18}") +
                strPurple(f"{formatted_bn_vol_diff:<18}") + 
                strBold(datetime.datetime.now().strftime("%d-%m-%Y %H:%M:%S"))
            , flush=True)

            # Save the current data to compare with the next iteration
            previous_nifty_data = [nifty_total_oi_CE, nifty_total_oi_PE, nifty_oi_difference, nifty_total_vol_CE, nifty_total_vol_PE, nifty_vol_difference]
            previous_banknifty_data = [banknifty_total_oi_CE, banknifty_total_oi_PE, banknifty_oi_difference, banknifty_total_vol_CE, banknifty_total_vol_PE, banknifty_vol_difference]
        ...
        t.sleep(200)  # Sleep for 10 seconds before running again
